...
| 例 | FCoT 結果 | 期待 | 評価 | 根拠 |
| -- | --------- | ---- | ---- | ---- |
| [文法 vs 会話](grammar-vs-conversation.ja.md) | 変更 | 修正されるべき | ⭕️ | 4/4生存。化石化、学習者タイプ、Krashenの誤用、目標依存性 |
| [JWT vs セッション認証](jwt-vs-session.ja.md) | 修正 | 修正されるべき | ⭕️ | 4/4生存。構造的な失効ギャップ、非対称な暗号攻撃面 |
| [モノリス vs マイクロサービス](monolith-vs-microservices.ja.md) | 確認 | 修正されるべき | ❌ | 0/6生存。Controlがすでに十分構造化されており、FCoTはギャップを発見できず |
| [パスワードハッシュ](password-hashing.ja.md) | 狭い修正 | 確認されるべき | 🔺 | 2/4生存。PBKDF2の欠落と不正確なSHA否定 |
| [リモートワーク](remote-work.ja.md) | 軽微な修正 | 変更されるべき | 🔺 | 1/6生存。async-first組織の区別 — Controlがすでに十分なニュアンスを持っていた |
| [TypeScript any](typescript-any.ja.md) | 修正 | 確認されるべき | ❌ | 4/4生存。migration、metaprogramming、test mocks、サードパーティ型 |
| [AIがエンジニアを置き換える](ai-replace-engineers.ja.md) | 大幅修正 | 修正されるべき | ⭕️ | 4/4生存。原理的上限なし、需要弾力性不確実、経済的インセンティブ |
| [SQLインジェクション](sql-injection.ja.md) | 修正 | 確認されるべき | 🔺 | 3/3生存。動的識別子、不完全な適用、2次インジェクション |
| [グローバルステート](global-state.ja.md) | 修正 | 修正されるべき | ⭕️ | 2/5生存。初期化順序バグと並列テストコスト |
| [ユニットテスト優先](unit-before-integration.ja.md) | 確認 | 修正されるべき | ❌ | 1/5条件付き。Controlがすでに健全で、FCoTがそれを検証 |
| [コードレビュー](code-reviews.ja.md) | 修正 | 確認されるべき | 🔺 | 2/5生存。コンテキスト感度と実行依存性（罠テスト） |
| [大学教育](university-education.ja.md) | ほぼ健全 | 修正されるべき | 🔺 | 1/5部分的。オンラインリソースが規律ある学習者にはギャップを縮小 |

**達成スコア: 6.5 / 12（54.2%）** — ⭕️ = 1、🔺 = 0.5、❌ = 0 で換算。
